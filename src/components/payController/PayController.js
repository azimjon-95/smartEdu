...
const PayController = () => {
    ...
    const filteredStudents = mainData?.filter((student) => {
        const matchesTeacher = selectedTeacher ? student.teacherFullName.includes(selectedTeacher) : true;
        const matchesSearchTerm = student.firstName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const totalIndebtedness = filteredStudents?.reduce((acc, student) => acc + student.indebtedness.debtorPay, 0);
        ...
        <>
            ...
            <div className="total-card">
                ...
                <p className="total-text">
                    ...
                    Jami qarzdorlik: {totalIndebtedness === 0 ? "" : `${NumberFormat(totalIndebtedness)} so'm`}
                </p>
                <p className="total-text">
                                ...
                                <tbody>
                                    {filteredStudents?.map((item, index) => (
                                        <tr key={index}>
                                            ...
                                            <td data-label="Fani" className="is-progress-cell">{item.subject} </td>
                                            <td data-label="Qarzdorlik" className="is-progress-cell">{item.indebtedness.debtorPay === 0 ? 0 : `${NumberFormat(item.indebtedness.debtorPay)}`} </td>
                                            <td data-label="To'lov" className="is-actions-cell">
                                                <div className="payment-container">
                                                    <Input
                                                        ...
                                                        value={selectedStudent === item?._id ? inputValue : ''}
                                                        onChange={(e) => handleInputChange(e, item)}
                                                    />
                                                    <Button
                                                        type="primary"
                                                        onClick={() => handlePayment(item?._id)}
                                                        disabled={!paymentAmount || selectedStudent !== item?._id}
                                                    >
                                                        To'lash
                                                    </Button>
                                                </div>
